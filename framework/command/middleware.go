	...
	RunE: func(c *cobra.Command, args []string) error {
		...
		filepath.Walk(repoFolder, func(path string, info os.FileInfo, err error) error {
			...
			if isContain {
				...
				c = bytes.ReplaceAll(c, []byte("github.com/gin-gonic/gin"), []byte("github.com/sherlockjjobs/sigma/framework/gin"))
...
import "github.com/sherlockjjobs/sigma/framework/gin"
