...
import (
	...
	"github.com/sherlockjjobs/sigma/framework/util"
)
...
var newCommand = &cobra.Command{
	...
	RunE: func(c *cobra.Command, args []string) error {
		...
		}
		{
			...
			prompt := &survey.Input{
				Message: "请输入版本名称(参考 https://github.com/sherlockjjobs/sigma/releases，默认为最新版本)：",
			}
			...
			if version != "" {
				...
				if err != nil || release == nil {
					fmt.Println("版本不存在，创建应用失败，请参考 https://github.com/sherlockjjobs/sigma/releases")
					return nil
				}
			}
			if version == "" {
		...
		filepath.Walk(folder, func(path string, info os.FileInfo, err error) error {
			...
			if path == filepath.Join(folder, "go.mod") {
				...
				c = bytes.ReplaceAll(c, []byte("module github.com/sherlockjjobs/sigma"), []byte("module "+mod))
				c = bytes.ReplaceAll(c, []byte("require ("), []byte("require (\n\tgithub.com/sherlockjjobs/sigma "+version))
			...
			isContain := bytes.Contains(c, []byte("github.com/sherlockjjobs/sigma/app"))
			if isContain {
				fmt.Println("更新文件:" + path)
				c = bytes.ReplaceAll(c, []byte("github.com/sherlockjjobs/sigma/app"), []byte(mod+"/app"))
				err = ioutil.WriteFile(path, c, 0644)
				if err != nil {
					return err
				}
